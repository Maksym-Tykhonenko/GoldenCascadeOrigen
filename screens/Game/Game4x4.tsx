...
const ITEM_SIZE = wp(20); // Для адаптивного размера иконок
const ITEM_MARGIN = wp(1.5); // Для адаптивных отступов между иконками

const Game4x4Screen = ({navigation}: NavProps) => {
  const [icons, setIcons] = useState<Icon[][]>(initialIcons4x4);
  ...
  useEffect(() => {
    loadPoints().then(data => setPoints(data ?? '0'));
    checkForMatches();
  }, [icons]);
...
const styles = StyleSheet.create({
  ...
  title: {
    ...
    color: '#E6E6E6',
    textAlign: 'center',
    marginTop: hp(8), // Адаптивный отступ сверху
    fontWeight: '800',
  },
  subtitle: {
